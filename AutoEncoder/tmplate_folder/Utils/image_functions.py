...
def image_prepare(img,size,interpolation = 'cubic'):
    ...
    img=np.array(Image.fromarray(img).resize(size=(size,size)))
...
def image_collage(img_arrays, rows =10, border =5,save_file = None):
    ...
    img_len =img_arrays[1].shape[2]
    array_len  = img_arrays[1].shape[0]
    ...
    if(save_file is not None):
        save_img(save_file,img_collage)
...
def save_images(images,images_orig = None ,folder=''):
    ...
    for i in range(images.shape[0]):
        if(images_orig is None):
            ...
            save_img(folder + 'img_' + str(i) + '.jpg', img_concat)