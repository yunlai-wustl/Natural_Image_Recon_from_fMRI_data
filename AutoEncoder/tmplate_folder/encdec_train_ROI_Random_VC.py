...
Y,Y_test,Y_test_avg = handler.get_data(roi = 'ROI_Random_VC',imag_data = 0) ##'img_data=0' means not returning images
labels_train, labels = handler.get_labels(imag_data = 0)
#print(labels_train)
#print(labels)
##print("sum of test avg")
##print(np.sum(np.sum(Y_test_avg)))
##print("shape of test avg")
##print(Y_test_avg.shape)
##print("var of Y_test,axis=0")
##print(np.var(Y_test_avg, axis=0))
##print("var of Y_test,axis=1")
##print(np.var(Y_test_avg, axis=1))
...
def combined_voxel_loss(y_true, y_pred):
    return mae_vox(y_true, y_pred) +  0.1 *cosine_similarity(y_true, y_pred)
